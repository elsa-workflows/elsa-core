...
namespace Elsa.Activities.Kafka.Services
{
    public class Worker : IAsyncDisposable
    {
        ...
        private async Task TriggerWorkflowsAsync(KafkaMessageEvent ev, CancellationToken cancellationToken)
        {
            ...
            var workflowInput = new WorkflowInput(new MessageReceivedInput() { MessageBytes = ev.Message.Value, MessageString = Encoding.ASCII.GetString(ev.Message.Value), MessageHeaders = JsonConvert.SerializeObject(GetHeaders(ev.Message.Headers,false)) });
