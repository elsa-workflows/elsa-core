...
namespace Elsa.Activities.AzureServiceBus.Consumers
{
    public class RestartServiceBusTopicsConsumer : IHandleMessages<TriggerIndexingFinished>, IHandleMessages<TriggersDeleted>, IHandleMessages<BookmarkIndexingFinished>, IHandleMessages<BookmarksDeleted>
    {
        ...
        public async Task Handle(TriggerIndexingFinished message) => await _serviceBusTopicsStarter.CreateWorkersAsync(message.Triggers, message.Tenant);
        public async Task Handle(TriggersDeleted message) => await _serviceBusTopicsStarter.RemoveWorkersAsync(message.Triggers);
        public async Task Handle(BookmarkIndexingFinished message) => await _serviceBusTopicsStarter.CreateWorkersAsync(message.Bookmarks, message.Tenant);
        public async Task Handle(BookmarksDeleted message) => await _serviceBusTopicsStarter.CreateWorkersAsync(message.Bookmarks, message.Tenant);
    }
}