...
import {WorkflowType, Activity, Endpoint, Transition} from "./workflow-models";
...
export class WorkflowEditor {
    ...
    readActivities() {
        ...
        for (let index = 0; index < activityElements.length; index++) {
            ...
            const activity = activityElement.data['activity-json'];
    ...
    addSourceEndpoints(activity, activityElement){
        for(let i = 0; i < activity.endpoints.length; i++)
        {
            const endpoint = activity.endpoints[i];
            const sourceEndpointOptions = WorkflowEditor.getSourceEndpointOptions(activity.id, endpoint.name);
    ...
    static getSourceEndpointOptions(activityId, endpointName) {
    ...
    static areEqualOutcomes(outcomes1, outcomes2) {
