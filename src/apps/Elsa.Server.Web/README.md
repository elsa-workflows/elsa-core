...
In the reference server, `Enabled` turns on the rate limiter middleware and registers the default fixed-window policies unless `IngressRateLimiting:RegisterReferencePolicies` is set to `false`. Externally configured policy names are preserved; the defaults are only assigned when the corresponding option is unset. Set a policy option to an empty string to explicitly disable rate limiting for that surface even when reference policies are registered.
