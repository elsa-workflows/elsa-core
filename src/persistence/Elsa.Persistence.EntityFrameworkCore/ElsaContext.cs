...
namespace Elsa.Persistence.EntityFrameworkCore
{
    public class ElsaContext : DbContext
    {
        ...
        private void ConfigureWorkflowInstance(ModelBuilder modelBuilder)
        {
            ...
            entity.Property(x => x.Activities).HasConversion(x => Serialize(x), x => Deserialize<IDictionary<string, ActivityInstance>>(x));
            ...
            entity.Property(x => x.ExecutionLog).HasConversion(x => Serialize(x), x => Deserialize<ICollection<LogEntry>>(x));
