...
namespace Elsa.Bookmarks
{
    public abstract class BookmarkProvider<T, TActivity> : IBookmarkProvider where T : IBookmark where TActivity : IActivity
    {
        ...
        public virtual ValueTask<IEnumerable<IBookmark>> GetBookmarksAsync(BookmarkProviderContext<TActivity> context, CancellationToken cancellationToken) => new(GetTriggers(context));
        public virtual IEnumerable<IBookmark> GetTriggers(BookmarkProviderContext<TActivity> context) => Enumerable.Empty<IBookmark>();
        ...
        async ValueTask<IEnumerable<IBookmark>> IBookmarkProvider.GetBookmarksAsync(BookmarkProviderContext context, CancellationToken cancellationToken)
        {
            ...
            return await GetBookmarksAsync(new BookmarkProviderContext<TActivity>(context.ActivityExecutionContext), cancellationToken);
        }
    }
}