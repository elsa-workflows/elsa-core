...
public class ProcessLogActivity : CodeActivity
{
    ...
    /// <inheritdoc />
    public ProcessLogActivity(string message, LogLevel level = LogLevel.Information, [CallerFilePath] string? source = null, [CallerLineNumber] int? line = null) 
        : base(source, line)
    {
        Message = new Input<string>(message);
        Level = new Input<LogLevel>(level);
    }

    /// <inheritdoc />
    public ProcessLogActivity(Input<string> message, Input<LogLevel> level, [CallerFilePath] string? source = null, [CallerLineNumber] int? line = null) 
        : base(source, line)
    {
        Message = message;
        Level = level;
