...
public interface IEventPublisher
{
    ...
    /// </summary>
    Task PublishAsync(string eventName, string? correlationId = default, IDictionary<string, object>? input = default, CancellationToken cancellationToken = default);
    
    /// <summary>
    /// Asynchronously publishes the specified event using the workflow dispatcher.
    /// </summary>
    Task DispatchAsync(string eventName, string? correlationId = default, IDictionary<string, object>? input = default, CancellationToken cancellationToken = default);
}