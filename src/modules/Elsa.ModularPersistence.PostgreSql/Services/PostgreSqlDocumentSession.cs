...
/// <summary>
/// Executes PostgreSQL document operations.
/// </summary>
public sealed class PostgreSqlDocumentSession(IRelationalConnectionFactory connectionFactory, StorageManifestDescriptor manifest) : IDocumentSession
{
    ...
    public async ValueTask<DocumentEnvelope?> LoadAsync(DocumentKey key, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            SELECT Id, Type, TenantId, Version, CreatedAt, UpdatedAt, Data::text, Metadata::text
    ...
    private static void ConfigureQueryCommand(DbCommand command, StorageUnitDescriptor storageUnit, DocumentQuery query)
    {
        ...
        var orderBy = query.Sorts.Count == 0
            ? query.Page is null ? "" : $"{Environment.NewLine}ORDER BY d.Id ASC"
        ...
        command.CommandText = $"""
            SELECT d.Id, d.Type, d.TenantId, d.Version, d.CreatedAt, d.UpdatedAt, d.Data::text, d.Metadata::text
    ...
    private static string BuildFilterCondition(DbCommand command, string alias, DocumentQueryFilter filter, StorageFieldDescriptor field, int filterIndex)
    {
        ...
        return filter.Operator switch
        {
            ...
            DocumentQueryFilterOperator.IsNull => $"{alias}.NullValue = TRUE",
            DocumentQueryFilterOperator.IsNotNull => $"{alias}.NullValue = FALSE",
            DocumentQueryFilterOperator.StartsWith => throw new NotSupportedException("PostgreSQL document queries do not support starts-with in the portable MVP."),
    ...
    private static object? ConvertQueryValue(DocumentQueryValue value, StorageFieldType fieldType) =>
        fieldType switch
        {
            ...
            StorageFieldType.Boolean => value.BooleanValue,
    ...
    private static async ValueTask InsertDocumentAsync(DbConnection connection, DbTransaction transaction, DocumentEnvelope document, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            ...
            VALUES (@Id, @Type, @TenantId, @Version, @CreatedAt, @UpdatedAt, CAST(@Data AS jsonb), CAST(@Metadata AS jsonb));
            """;
    ...
    private static async ValueTask UpdateDocumentAsync(DbConnection connection, DbTransaction transaction, DocumentEnvelope document, ExpectedDocumentVersion expectedVersion, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = $"""
            ...
            SET Version = @Version, CreatedAt = @CreatedAt, UpdatedAt = @UpdatedAt, Data = CAST(@Data AS jsonb), Metadata = CAST(@Metadata AS jsonb)
        ...
        CancellationToken cancellationToken)
    {
        ...
        AddParameter(command, "@NullValue", value.IsNull);
        await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    private sealed record IndexedValue(string? StringValue, double? NumberValue, bool? BooleanValue, string? DateTimeValue, bool IsNull)
    {
        ...
        public static IndexedValue Boolean(bool value) => new(null, null, value, null, false);
