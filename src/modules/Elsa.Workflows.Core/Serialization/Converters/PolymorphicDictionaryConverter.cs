...
public class PolymorphicDictionaryConverter : JsonConverter<IDictionary<string, object>>
{
    ...
    /// <inheritdoc />
    public PolymorphicDictionaryConverter(JsonSerializerOptions options, WorkflowJsonOptions workflowJsonOptions)
    {
        var factory = (JsonConverterFactory)(options.Converters.FirstOrDefault(x => x is PolymorphicObjectConverterFactory) ?? new PolymorphicObjectConverterFactory(workflowJsonOptions));
