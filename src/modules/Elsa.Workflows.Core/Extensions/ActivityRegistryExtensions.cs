...
public static class ActivityRegistryExtensions
{
    ...
    extension(IActivityRegistry activityRegistry)
    {
        /// <summary>
        /// Finds the activity descriptor for the specified activity type.
        /// </summary>
        public ActivityDescriptor? Find<T>() where T:IActivity => activityRegistry.Find(ActivityTypeNameHelper.GenerateTypeName<T>());
        ...
        public async Task RegisterAsync<T>(CancellationToken cancellationToken = default) where T : IActivity => 
