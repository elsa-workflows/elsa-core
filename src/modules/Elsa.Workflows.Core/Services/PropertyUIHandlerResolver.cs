...
public class PropertyUIHandlerResolver(IServiceScopeFactory scopeFactory) : IPropertyUIHandlerResolver
{
    ...
    public async ValueTask<IDictionary<string, object>> GetUIPropertiesAsync(PropertyInfo propertyInfo, object? context, CancellationToken cancellationToken = default)
    {
        ...
        {
            IPropertyUIHandler provider;
            if (propertyInfo.DeclaringType?.Assembly.IsCollectible ?? false)
                provider = (IPropertyUIHandler)ActivatorUtilities.CreateInstance(scope.ServiceProvider, handlerType);
            else
                provider = (IPropertyUIHandler)ActivatorUtilities.GetServiceOrCreateInstance(scope.ServiceProvider, handlerType);
