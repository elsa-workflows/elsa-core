...
/// <summary>
/// Executes SQL Server document operations.
/// </summary>
public sealed class SqlServerDocumentSession(IRelationalConnectionFactory connectionFactory, StorageManifestDescriptor manifest) : IDocumentSession
{
    ...
    public async ValueTask<DocumentEnvelope?> LoadAsync(DocumentKey key, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            ...
            FROM dbo.ModularPersistenceDocuments
    ...
    private static void ConfigureQueryCommand(DbCommand command, StorageUnitDescriptor storageUnit, DocumentQuery query)
    {
        ...
        if (query.Page is not null)
        {
            ...
            paging = $"{Environment.NewLine}OFFSET @Offset ROWS FETCH NEXT @Limit ROWS ONLY";
        }

        command.CommandText = $"""
            SELECT d.Id, d.Type, d.TenantId, d.Version, d.CreatedAt, d.UpdatedAt, d.Data, d.Metadata
            FROM dbo.ModularPersistenceDocuments d
    ...
    private static string BuildFilterExistsClause(DbCommand command, DocumentQueryFilter filter, StorageFieldDescriptor field, int filterIndex)
    {
        ...
        return $"""
            EXISTS (
                SELECT 1
                FROM dbo.ModularPersistenceDocumentIndexes {alias}
    ...
    private static string BuildFilterCondition(DbCommand command, string alias, DocumentQueryFilter filter, StorageFieldDescriptor field, int filterIndex)
    {
        ...
        return filter.Operator switch
        {
            ...
            DocumentQueryFilterOperator.StartsWith => throw new NotSupportedException("SQL Server document queries do not support starts-with in the portable MVP."),
    ...
    private static string BuildSortExpression(DbCommand command, StorageUnitDescriptor storageUnit, DocumentQuerySort sort, int sortIndex)
    {
        ...
        return $"""
            (
                SELECT TOP (1) {column.For("s" + sortIndex)}
                FROM dbo.ModularPersistenceDocumentIndexes s{sortIndex}
    ...
    private static async ValueTask InsertDocumentAsync(DbConnection connection, DbTransaction transaction, DocumentEnvelope document, CancellationToken cancellationToken)
    {
        ...
        command.Transaction = transaction;
        command.CommandText = """
            INSERT INTO dbo.ModularPersistenceDocuments (Id, Type, TenantId, Version, CreatedAt, UpdatedAt, Data, Metadata)
    ...
    private static async ValueTask UpdateDocumentAsync(DbConnection connection, DbTransaction transaction, DocumentEnvelope document, ExpectedDocumentVersion expectedVersion, CancellationToken cancellationToken)
    {
        ...
        command.Transaction = transaction;
        command.CommandText = """
            UPDATE dbo.ModularPersistenceDocuments
    ...
    private static async ValueTask DeleteDocumentAsync(DbConnection connection, DbTransaction transaction, DocumentKey key, ExpectedDocumentVersion expectedVersion, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = $"""
            DELETE FROM dbo.ModularPersistenceDocuments
    ...
    private static async ValueTask DeleteIndexEntriesAsync(DbConnection connection, DbTransaction transaction, DocumentKey key, CancellationToken cancellationToken)
    {
        ...
        command.Transaction = transaction;
        command.CommandText = """
            DELETE FROM dbo.ModularPersistenceDocumentIndexes
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.Transaction = transaction;
        command.CommandText = """
            INSERT INTO dbo.ModularPersistenceDocumentIndexes (
                DocumentId,
                DocumentType,
                TenantId,
                IndexName,
