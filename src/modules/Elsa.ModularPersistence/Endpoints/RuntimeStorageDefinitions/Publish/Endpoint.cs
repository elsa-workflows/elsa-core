...
namespace Elsa.ModularPersistence.Endpoints.RuntimeStorageDefinitions.Publish;
...
internal sealed class Endpoint(IRuntimeStorageDefinitionManager manager) : ElsaEndpoint<PublishRuntimeStorageDefinitionRequest, RuntimeStorageDefinitionPublishResponse>
{
    ...
    {
        Post("/admin/modular-persistence/runtime-storage-definitions/{id}/publish");
    ...
    {
        var result = await manager.PublishAsync(Route<string>("id")!, request.ProviderName, EndpointContext.Create(User), cancellationToken);
