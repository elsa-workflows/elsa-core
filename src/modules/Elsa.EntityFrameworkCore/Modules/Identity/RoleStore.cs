...
/// </remarks>
public class EFCoreRoleStore(EntityStore<IdentityElsaDbContext, Role> applicationStore) : IRoleStore
{
    private readonly EntityStore<IdentityElsaDbContext, Role> _applicationStore = applicationStore;
    ...
    public async Task<IEnumerable<Role>> FindManyAsync(RoleFilter filter, CancellationToken cancellationToken = default) =>
        await _applicationStore.QueryAsync(queryable => Filter(queryable, filter), cancellationToken).ToList();
