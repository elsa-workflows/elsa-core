...
public static class OtlpIngestionSecurity
{
    public static bool IsAuthorized(HttpContext httpContext, OpenTelemetryDiagnosticsOptions options)
    {
        ...
        return httpContext.Request.Headers.TryGetValue(options.ApiKeyHeaderName, out var value) && string.Equals(value.ToString(), options.ApiKey, StringComparison.Ordinal);
    }
