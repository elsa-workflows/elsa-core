...
3. For documentation, prefer the document connector. If it is unavailable, use the signed-in browser fallback against the exact `elsa-workflows/elsa-gitbook` `main` target. Create or update the normal docs PR, review the diff, and integrate it through the repository's ordinary checks and branch policy. A PR being merged is an update identifier, not production verification.
...
5. Publish or integrate the reviewed update, then verify the live production URL. Capture the version shown by the deployed page, every changed public URL on the configured production origin, the deployment ID or commit, and a past UTC evidence timestamp. Save the receipt before doing anything else.
6. Resume from the saved receipt and operation/message/project IDs. If an update response is uncertain, reconcile the exact target and existing operation before retrying; do not submit the same prompt or PR again merely because the connector timed out. An uncertain create is resolved by lookup, never by a blind duplicate.
