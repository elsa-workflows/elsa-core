...
The pinned 95a rehearsal contains the earlier canonical Workbench Secrets patch (SHA-256 `5dd4667190c538279f3b89cea80b81681402775555a89d89248001c2e22aa446`). A newer rehearsal may instead carry the byte-exact current patch (SHA-256 `eb792ede2d40eaae72f8b6174ba920c8e1ee02e71e2506c4905737245d1c313c`). The runtime validator accepts only these reviewed artifact hashes and reverses the complete mapped patch chain to verify the source baseline. For the older state, it replays the old patch, reverses it, and applies the current patch; for the already-current state, it replays the current patch directly. Follow the verified `patchTransition` in the generated launch plan. Apply either transition only in the private copy-on-write clone, never in the canonical rehearsal. The reviewed runtime delta makes Secrets opt-in through `Features:Secrets:Enabled`, whose default remains `false`. The source receipt hash refers to the external preparer's integration patch, not the older copy of that tool stored inside the mapped rehearsal.
