...
{
    private const string Phase = "current-core-sqlite-bridge";
    ...
    public static async Task<int> Main(string[] args)
    {
        if (args.Length != 7)
        {
            Console.Error.WriteLine("Usage: CurrentCoreBridgeRunner <source-db> <target-db> <old-key-ring> <wrong-key-ring> <missing-key-ring> <tenant-map.json> <success|id-collision|fail-after-core-save>");
            return 64;
        }
        ...
        catch (BridgeRejectedException error)
        {
            Console.WriteLine(JsonSerializer.Serialize(new
            {
                ...
                targetUnchanged = error.TargetUnchanged,
    ...
    private static async Task<object> RunAsync(string[] args)
    {
        ...
        try
        {
            var source = await ReadSourceAsync(sourcePath);
            var groups = Preflight(source, tenantMap, await ReadExistingTargetAsync(services));
        ...
        catch (BridgeRejectedException error)
        {
            ...
            throw new BridgeRejectedException(error.Code, unchanged);
        }
    }
    ...
    private static ServiceProvider CreateCurrentServices(string targetPath, DefaultTenantAccessor tenantAccessor)
    {
        ...
        var services = new ServiceCollection()
            ...
            .AddDbContextFactory<SecretsElsaDbContext>(builder => builder
                ...
                .UseElsaSqlite(typeof(SqliteSecretsPersistenceFeatureExtensions).Assembly, connectionString))
    ...
    private static async Task<List<LegacyRow>> ReadSourceAsync(string path)
    {
        var connectionString = new SqliteConnectionStringBuilder
        {
            DataSource = path,
        ...
        await using (var command = connection.CreateCommand())
        {
            ...
            {
                var createdAtRaw = reader.GetString(13);
                var updatedAtRaw = reader.GetString(14);
        ...
        string scenario)
    {
        ...
        foreach (var row in source)
        {
            ...
            AddParameter(command, "@legacyIsLatest", row.IsLatest ? 1 : 0);
    ...
    private const string CreateSidecarSql = """
        ...
        CREATE INDEX IX_ElsaSecretsLegacyV381_SecretId ON ElsaSecretsLegacyV381 (LegacySecretId);
        CREATE INDEX IX_ElsaSecretsLegacyV381_TenantOwner ON ElsaSecretsLegacyV381 (LegacyTenantId, LegacyOwner);
        """;

    private const string InsertSidecarSql = """
        INSERT INTO ElsaSecretsLegacyV381 (
    ...
    private static async Task<bool> VerifySidecarAsync(IServiceProvider services, IReadOnlyList<LegacyRow> source, IReadOnlyList<Secret> secrets)
    {
        ...
        command.CommandText = """
            ...
            FROM ElsaSecretsLegacyV381 ORDER BY LegacyId;
            """;
        await using var reader = await command.ExecuteReaderAsync();
        ...
        var expected = source.ToDictionary(
            ...
            row => new[]
            {
                ...
                row.Version.ToString(CultureInfo.InvariantCulture), row.IsLatest ? "1" : "0", row.Status.ToString(CultureInfo.InvariantCulture),
        ...
        DefaultSecretValueProtector coreProtector)
    {
        ...
        return new EncryptionResult(
            ...
            wrongCoreKeyRejected && missingCoreKeyRejected,
    ...
    private static IDataProtectionProvider CreateDataProtectionProvider(string keyRingPath)
    {
    ...
    private const string SidecarExistsSql = "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='ElsaSecretsLegacyV381'";
    ...
    private sealed record EncryptionResult(bool RoundTrip, bool WrongCoreKeyRejected, bool RawLegacyCiphertextRejected);

    private sealed class BridgeRejectedException(string code, bool targetUnchanged = true) : Exception
    {
        public string Code { get; } = code;
        public bool TargetUnchanged { get; } = targetUnchanged;
    }
}
