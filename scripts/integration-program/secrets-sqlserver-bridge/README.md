...
The fixture pins the released package versions and SHA-512 values, source commits, and SQL Server container digest in `artifacts.json`. The Core runner is built from the pinned source commit in a temporary checkout with the committed `current-core/packages.lock.json` restored in locked mode. The lock was regenerated under Linux with the pinned SDK 10.0.300 after the macOS-generated lock omitted project-reference entries and failed Ubuntu restore with `NU1004`. It contains 75 package entries and 21 project-reference entries; the report records its SHA-256 and both counts. Locked restore remains mandatory on every normal run. A local package-manifest tooling package is included because that pinned Core source build requires it; its source commit and SHA-512 are recorded in the manifest and independently verified before restore.
