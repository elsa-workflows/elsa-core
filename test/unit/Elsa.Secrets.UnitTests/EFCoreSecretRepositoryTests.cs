...
public class EFCoreSecretRepositoryTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task LegacyConstructor_UsesTenantFilteredContextForOwnershipGuards()
    {
        await WithTenantAwareRepositoryAsync(async (repository, tenantAccessor) =>
        {
            using (UseTenant(tenantAccessor, Tenant.AgnosticTenantId))
                await repository.SaveAsync(new Secret { Name = "shared:secret", DisplayName = "Shared", TenantId = Tenant.AgnosticTenantId });
            ...
            using (UseTenant(tenantAccessor, "tenant-a"))
            {
                ...
                Assert.Equal("A secret named 'SHARED:SECRET' belongs to another tenant.", exception.Message);
            }
