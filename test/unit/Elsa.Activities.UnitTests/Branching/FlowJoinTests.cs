...
{
    [Test]
    [Arguments(FlowchartExecutionMode.TokenBased, FlowJoinMode.WaitAny)]
    [Arguments(FlowchartExecutionMode.TokenBased, FlowJoinMode.WaitAll)]
    [Arguments(FlowchartExecutionMode.CounterBased, FlowJoinMode.WaitAny)]
    [Arguments(FlowchartExecutionMode.CounterBased, FlowJoinMode.WaitAll)]
    ...
    public async Task Should_Execute_In_Flowchart_Context_For_All_Combinations(FlowchartExecutionMode executionMode, FlowJoinMode joinMode)
    {
        ...
        await Assert.That(context.HasScheduledActivity(flowJoin)).IsTrue();
    }
    ...
    public async Task Should_Demonstrate_Token_vs_Counter_Flow_Differences()
    {
        ...
        // Assert
        await Assert.That(tokenContext.IsCompleted).IsTrue().Because("Token flow should always complete");
        await Assert.That(counterContext.IsCompleted).IsTrue().Because("Counter flow should complete without parent context");
    }
