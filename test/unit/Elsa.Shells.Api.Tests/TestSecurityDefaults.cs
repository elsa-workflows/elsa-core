...
/// once per host, while <c>MapFastEndpoints()</c> configures the endpoints. TUnit runs the test classes in this
/// assembly in parallel, so a per-test save/restore of that global races: one class restoring the flag to
/// <c>true</c> between another class's write and its <c>MapFastEndpoints()</c> call produces endpoints carrying
