...
/// <summary>
/// Tests that <see cref="HangfireWorkflowScheduler.UnscheduleAsync"/> removes recurring jobs for both
/// new-instance (<see cref="RunWorkflowJob"/>) and existing-instance (<see cref="ResumeWorkflowJob"/>) schedules.
/// </summary>
public class HangfireWorkflowSchedulerTests
{
    ...
    public async Task UnscheduleAsync_RemovesResumeWorkflowJobScheduledViaScheduleRecurringAsync()
    {
        ...
        await sut.ScheduleRecurringAsync("task-1", request, DateTimeOffset.UtcNow, TimeSpan.FromMinutes(5));
        Assert.Contains(GetRecurringJobs(storage), job => job.Id == "task-1" && job.Job.Type == typeof(ResumeWorkflowJob));
