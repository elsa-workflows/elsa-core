...
Last refreshed: 2026-09-23 (3.8.4 package-integrity patch and atomic activation admission)
...
- [~] Atomic workflow admission for built-in `Singleton`, `CorrelatedSingleton`, and `Correlation` activation strategies is now on `main`, including a distributed lease across direct, trigger, and definition-dispatch starts; custom strategies, shared lock-provider configuration, release packaging, and operational verification remain ([#8179](https://github.com/elsa-workflows/elsa-core/pull/8179))
...
- [~] Studio user and role administration (released in the `3.8.4` train; operational verification and authoring-side governance remain)
- [~] Studio diagnostics pages for structured logs, console logs, and OpenTelemetry
- [~] Studio alterations module
- [~] User Tasks and human-workflow workbench: Core durable, identity-neutral tasks and the paired Studio workbench are released in the `3.8.4` train; realtime delivery, cross-provider conformance, samples, and operational validation remain
...
- [~] Azure DevOps connector implementation is under review; Teams, OneDrive, SharePoint, Google Docs, and Google Sheets still need a shared integration strategy ([elsa-extensions#199](https://github.com/elsa-workflows/elsa-extensions/pull/199))
...
- The `3.8.1` patch train shipped on September 12 across [Core](https://github.com/elsa-workflows/elsa-core/releases/tag/3.8.1), [Studio](https://github.com/elsa-workflows/elsa-studio/releases/tag/3.8.1), and [Extensions](https://github.com/elsa-workflows/elsa-extensions/releases/tag/3.8.1). It restores Studio-selectable activation strategies, corrects terminal-instance interrupted recovery, fixes Studio OIDC/static-feature dashboard/execution-detail regressions, and supplies the MongoDB/Dapper interruption-store companions. The aligned `3.8.2` train on September 15 is a critical package correction for MongoDB/Dapper hosts. Studio `3.8.3` then released the User Tasks workbench, BPMN Studio slice, user/role administration, output-converter authoring, and related fixes, but depended on an unavailable preview API client. The superseding [Core](https://github.com/elsa-workflows/elsa-core/releases/tag/3.8.4), [Studio](https://github.com/elsa-workflows/elsa-studio/releases/tag/3.8.4), and [Extensions](https://github.com/elsa-workflows/elsa-extensions/releases/tag/3.8.4) `3.8.4` train repairs that package-integrity break: use it instead of Studio `3.8.3`, and upgrade all three repositories together. These releases improve the shipped operational and authoring foundations; recovery, atomic-admission release packaging, and cross-provider confidence remain roadmap work.
...
- Core `main` now applies configured built-in exclusive activation strategies at the shared workflow-creation boundary, using a distributed lease through first durable persistence while preserving the default allow-always behavior ([#8179](https://github.com/elsa-workflows/elsa-core/pull/8179)). This closes [#7948](https://github.com/elsa-workflows/elsa-core/issues/7948) and [#8042](https://github.com/elsa-workflows/elsa-core/issues/8042), but is not yet a released package claim.
...
- Studio `3.8.4` has permission-aware user and role list, create, edit, and delete flows; role assignments; permission-token editing; tenant-scope display; guided role deletion, and permission-gated user administration ([elsa-studio#936](https://github.com/elsa-workflows/elsa-studio/pull/936), [#982](https://github.com/elsa-workflows/elsa-studio/pull/982)-[#984](https://github.com/elsa-workflows/elsa-studio/pull/984), [#990](https://github.com/elsa-workflows/elsa-studio/pull/990), [#996](https://github.com/elsa-workflows/elsa-studio/pull/996)). This is a partially shipped identity-administration foundation, not workflow-authoring governance: broader operational validation and documentation remain, and tenant/role-based activity visibility is still roadmap work.
...
- Productize the paired User Tasks implementation as a supportable human-workflow feature. Core supplies durable, identity-neutral workflow tasks, invitations/guest sessions, authorization, persistence, audit, and delivery reliability; Studio supplies the queue, task/manager workbench, guest completion page, and polling fallback in `3.8.4` ([#7955](https://github.com/elsa-workflows/elsa-core/pull/7955), [elsa-studio#941](https://github.com/elsa-workflows/elsa-studio/pull/941)). Prioritize a Core invalidation hub, cross-provider conformance coverage, samples, and operational validation.
...
- Productize activity output conversion across Core, Studio, and Extensions. Core supplies opt-in, typed, server-discoverable converter contracts, while Studio `3.8.4` provides selection and schema-driven settings editing ([#7902](https://github.com/elsa-workflows/elsa-core/pull/7902), [elsa-studio#922](https://github.com/elsa-workflows/elsa-studio/pull/922)). Add a small, maintained extension converter catalog instead of workflow-specific wrapper activities ([#7770](https://github.com/elsa-workflows/elsa-core/issues/7770), [elsa-extensions#164](https://github.com/elsa-workflows/elsa-extensions/issues/164)).
...
- Prioritize enterprise productivity integrations by leverage: Azure DevOps has moved from a proposal to an active module implementation ([elsa-extensions#124](https://github.com/elsa-workflows/elsa-extensions/issues/124), [elsa-extensions#199](https://github.com/elsa-workflows/elsa-extensions/pull/199)); treat it as a connector-SDK proving ground rather than a precedent for bespoke integrations. Teams, OneDrive, SharePoint, Google Docs, and Google Sheets should follow a shared connector model rather than separate designs.
...
1. Finish runtime confidence work: graceful shutdown remaining tasks, recovery clarity, atomic-admission release packaging and operational guidance, distributed runtime regressions, security documentation, and OIDC recipes.
...
6. Productize the BPMN runtime/interchange foundation: complete validation and conformance work, define the supported subset, validate the released Studio rendering/import/export/nested-authoring and guarded binding slice in configured hosts, then broaden visual editing and add instance inspection.
