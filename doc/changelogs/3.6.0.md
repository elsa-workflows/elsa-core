...
- **Workflow and activity telemetry**: Elsa now emits `Elsa.Workflows` `ActivitySource` spans for workflow execution cycles and activity execution, plus optional `Elsa.Workflows` meter instruments for workflow started/completed/faulted counts and activity duration. `SendHttpRequest` and `FlowSendHttpRequest` propagate the current W3C trace context on outbound HTTP requests when tracing is enabled. ([#7489](https://github.com/elsa-workflows/elsa-core/issues/7489))
