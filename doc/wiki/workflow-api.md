...
`WorkflowInstanceHub` carries `[Authorize]`, so every connection requires authentication. The `ObserveInstanceAsync` method additionally enforces `workflows/instances:view` (or wildcard `*:view` or `*:*`) and tenant access: callers can only observe workflow instances that belong to their current tenant or are tenant-agnostic.
