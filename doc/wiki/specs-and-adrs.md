...
| [0015](../adr/0015-compose-a-scoped-connection-registry.md) | Compose a scoped connection registry. |
| [0016](../adr/0016-extend-authentication-through-deployed-descriptor-providers.md) | Extend authentication through deployed descriptor providers. |
| [0017](../adr/0017-separate-external-identity-from-elsa-authorization.md) | Separate external identity from Elsa authorization. |
| [0018](../adr/0018-separate-provider-trust-from-broker-invariants.md) | Separate provider trust from broker invariants. |
| [0019](../adr/0019-bind-sessions-to-shared-state-and-connection-revisions.md) | Bind sessions to shared state and connection revisions. |
...
| [013 user tasks](../../specs/013-user-tasks/spec.md) | Workflow core | Durable workflow-bound human tasks: `UserTask` activity, candidate/assignee/supervisor roles, guest invitations, form resolution and validation, lifecycle management, REST surface, and persistence. |
| [013 RBAC authorization model](../../specs/013-rbac-authorization-model/spec.md) | Security | Two-axis `{resource}:{verb}` permission vocabulary, open resource and verb sets, permission descriptor catalog, pattern-based wildcard grants, per-module resource contributions, and the migration from the legacy `verb:resource` strings. |
...
1. [doc/user-tasks.md](../user-tasks.md) — feature overview
2. [specs/013-user-tasks/spec.md](../../specs/013-user-tasks/spec.md)
3. [specs/013-user-tasks/plan.md](../../specs/013-user-tasks/plan.md)
4. [ADR 0026](../adr/0026-identity-neutral-user-task-participants.md) — identity-neutral participant references
5. [ADR 0027](../adr/0027-project-user-tasks-from-committed-bookmarks.md) — bookmark projection
6. `Elsa.UserTasks` feature, contracts, and REST endpoints
7. `Elsa.UserTasks.Persistence.EFCore` and provider packages

## Reading Order For Authorization Model Work

1. [doc/migrations/authorization-model.md](../migrations/authorization-model.md) — migration guide and full permission mapping table
2. [specs/013-rbac-authorization-model/spec.md](../../specs/013-rbac-authorization-model/spec.md)
3. [ADR 0025](../adr/0025-two-axis-authorization-model.md) — two-axis model rationale
4. [Identity, Tenancy, And Security](identity-tenancy-security.md) — API authorization section
5. `Elsa.Authorization` contracts (resource/verb model and `PermissionMatcher`)
6. per-module `*ResourcePermissions` constant files
