...
| Async/await correctness | ✅ | `ConfigureAwait.Fody` is in use — `src/Fody.props` declares a single `ContinueOnCapturedContext="false"` directive that reaches every props root under `src/`, and `Directory.Build.targets` fails the build (`ELSA0001`/`ELSA0002`) for any project that references the weaver without an effective directive. `ConfigureAwait(false)` is injected automatically at build time; manual calls are not required, and the 25 explicit calls that remain in `src/` are redundant rather than load-bearing. The presence of a `FodyWeavers.xml` is *not* evidence of weaving — a bare `<ConfigureAwait />` element parses cleanly and rewrites nothing — so the directive and the build guard are what to check. No `.Result` blocking calls on live `Task`/`ValueTask` instances found (the `t.Result` in `BulkCancel/Endpoint.cs` accesses a completed `Task<int>` after `await Task.WhenAll(tasks)` — safe). Three `async void` methods found: `ScheduledTimer.Callback`, `HeartbeatGenerator.GenerateHeartbeatAsync`, `ConfigurationTenantsProvider.OnOptionsChanged` — all are timer or `IOptionsMonitor` callbacks where `async void` is the required signature, each wrapping a try/catch. |
