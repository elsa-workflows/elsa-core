...
The full build exposed missing `IWorkflowDefinitionStore.TryUpdateLatestAsync` implementations in Dapper and MongoDB. These remain blockers. They require real tenant-scoped database compare-and-swap behavior, including atomic old/new version transitions; a throwing stub or process-local lock would not satisfy the contract. The first forced-net10 diagnostic was invalid for the MySQL target matrix and was stopped; its partial output is not validation evidence.
