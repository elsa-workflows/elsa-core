...
The added-descriptor guard was tightened after review. [The comparison replay](activity-compatibility-evidence/added-descriptor-review.json) verifies the new Python guard against the unchanged, hash-checked C# evidence for all three frameworks: only the nine exact CLR/type/version identities recorded for LDAP/MQTT in the matrix are allowed additions. Additional or changed identities fail even within these source-only assemblies. The original run receipts retain their original harness hashes; no .NET rerun is claimed for this comparison-only change.
