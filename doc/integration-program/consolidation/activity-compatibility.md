...
The negative probes mutate an activity type, ID, version, literal input and first-write-only ID. Every case must fail the historical import check, while the unmodified fixture must pass that check first. Four Python comparison tests cover assembly build-version normalization, retained strong-name/contract changes, retained activity identities/ports, and duplicate CLR-name rejection; normal and optimized Python execution both pass.
