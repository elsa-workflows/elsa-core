...
The negative probes mutate an activity type, ID, version, literal input and first-write-only ID. Every case must fail the historical import check, while the unmodified fixture must pass that check first. Eight Python comparison tests cover assembly build-version normalization, retained strong-name/contract changes, retained activity identities/ports, duplicate CLR-name rejection, and rejection of added descriptors outside the matrix's nine reviewed source-only normalized descriptor contracts; normal and optimized Python execution both pass.
...
The added-descriptor guard was tightened after review. [The comparison replay](activity-compatibility-evidence/added-descriptor-review.json) verifies the new Python guard against the unchanged, hash-checked C# evidence for all three frameworks: only the nine complete normalized descriptor contracts recorded for LDAP/MQTT in the matrix are allowed additions. Additional identities or changes to their recorded kind, inputs, outputs or ports fail even within these source-only assemblies. The original run receipts retain their original harness hashes; no .NET rerun is claimed for this comparison-only change.
