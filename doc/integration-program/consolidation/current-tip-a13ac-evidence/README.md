...
The [source PR refresh](upstream-pr-refresh.json) records the selected Extensions and Studio tips and every still-open source PR head at its checked time. Both selected tips still matched source `main`. Extensions PR #216, present in the earlier ledger, had merged as the selected Extensions tip; the remaining 16 Extensions and 14 Studio PRs stayed open with unchanged head SHAs. Their disposition is to preserve them for owner review at the final import. This read-only refresh happened after the synthetic rehearsal was created and does not retroactively satisfy a pre-candidate check.
