...
The [source PR refresh](upstream-pr-refresh.json) records the selected Extensions and Studio tips and the still-open source PR heads returned at its checked time. It includes the exact GitHub REST collection paths (`state=open&per_page=100`, explicit pages 1 and 2), raw response hashes, native-field page snapshots, page counts, and a projection check against the disposition rows. Page 1 had fewer than 100 results and page 2 was empty for both repositories. Both selected tips still matched source `main`. Extensions PR #216, present in the earlier ledger, had merged as the selected Extensions tip; the remaining 16 Extensions and 14 Studio PRs stayed open with unchanged head SHAs. Their disposition is to preserve them for owner review at the final import. This point-in-time read-only refresh happened after the synthetic rehearsal was created and does not retroactively satisfy a pre-candidate check; repeat it before the canonical import.
