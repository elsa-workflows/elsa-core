...
`MongoWorkflowDefinitionStore.TryUpdateLatestAsync` reads the latest visible workflow row and its raw BSON snapshot in one MongoDB session transaction. It checks the expected-state callback, applies the update once, then conditionally replaces the exact full-document snapshot using simple collation for same-row updates. A published-to-draft transition uses an exact-snapshot `UpdateOne` that changes only `IsLatest`, then inserts the new draft in the same transaction; a raw BSON comparison verifies that no other published-row field changes. Tenant visibility comes from the filter's existing tenant-aware read path; the selected tenant ID is retained, and an update cannot change the logical definition ID. The original one-argument store constructor remains available.

The transaction uses snapshot reads, primary reads, and majority writes. It obtains the client from the collection's owning database, so a custom collection cannot accidentally be paired with a different injected client. The tested deployment is a MongoDB replica set; a standalone server cannot silently fall back to non-transactional behavior. Only MongoDB write-conflict code 112 is returned as `Conflict`; a transient-transaction label by itself is insufficient because it can identify infrastructure failures. Tests cover command error 112 as a known conflict and code 91 with the same transient label as a propagated error. The `matchesExpected` and `update` callbacks execute once and their exceptions propagate even when they carry MongoDB's transient-transaction label. Commit exceptions propagate because their outcome may be ambiguous; the implementation does not retry the callback or the transaction.
...
| Mongo compare-and-swap replica-set tests, net10.0 | 12 passed, 0 failed, 0 skipped on a fresh patch replay |
| Mongo persistence project, net8.0/net9.0/net10.0 | Passed, 0 errors, 50 disclosed warnings |

The replica-set tests cover same-row updates, two simultaneous writers released from a barrier after both read the same snapshot, stale metadata, callback exceptions carrying a transient label, tenant visibility and preservation, logical-definition identity, published-to-draft updates, and rollback when draft insertion fails. The successful published-to-draft test compares the raw prior row before and after and permits only `IsLatest=false`. A separate regression confirms that the default BSON class map rejects an unmapped field before mutation and leaves the raw document unchanged; this proof does not claim support for custom `IgnoreExtraElements` class maps. The two-writer case expects exactly one committed winner and one `Conflict` loser.
