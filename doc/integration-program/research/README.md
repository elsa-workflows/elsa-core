...
Run `python3 validate_catalog.py` from this directory after installing `jsonschema`. Run `python3 -m unittest test_validate_catalog.py` for regression cases covering duplicate, skipped and out-of-order workflow stages plus undeclared fields in nested contract objects; each invalid case must fail under standard Python and `python -O`. The validator checks the Draft 2020-12 schema, IDs and source references, claim provenance, provider-operation ownership in workflows, minimum catalog/deep counts, score arithmetic and ranking coverage.
