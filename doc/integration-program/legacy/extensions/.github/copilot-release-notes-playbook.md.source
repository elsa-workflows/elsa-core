# Copilot Playbook: Elsa Extensions Release Notes (GitHub Release + PR/commit links)

## Goal
Generate consistent GitHub Release notes for `elsa-workflows/elsa-extensions` between two tags/refs, including:
...
- Repository: `elsa-workflows/elsa-extensions`
- From tag/ref: `<FROM>`
- To tag/ref: `<TO>`

Also provide ONE of:
- `git log --oneline <FROM>..<TO>` output (preferred), OR
- compare URL: `https://github.com/elsa-workflows/elsa-extensions/compare/<FROM>...<TO>`
...
“Generate GitHub Release notes for `elsa-workflows/elsa-extensions` from `<FROM>` to `<TO>`. Use the template with **Highlights** and **Known issues** (only if I provide them). Prefer PR titles when available, otherwise use commit subjects. Use GitHub-style `(#NNNN)` references and include a short Full changelog. Here is `git log --oneline <FROM>..<TO>`: …”