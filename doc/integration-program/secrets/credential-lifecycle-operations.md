...
- Logical workflow bindings select a connection in the trusted tenant and configured integration environment; they do not grant use. A disconnect can race the binding store's active-state check and leave a stale mapping. Each resolve still passes through the lifecycle service, which rejects disconnected connections.
...
2. Take and verify restorable backups of every database containing connection rows, logical workflow bindings, or encrypted Secrets generations. Preserve the matching key material and application artifact/version with the backup record.
...
The Secrets schema adds nullable managed-owner and generation columns, and Connections adds durable lifecycle and workflow binding metadata. Existing Secrets rows remain unowned. Migration `Down` methods intentionally throw rather than silently erase lifecycle ownership, connection state, or workflow binding state, so there is no supported in-place downgrade path. Do not run an older application version against the upgraded schema, and do not manually drop ownership columns or connection/binding tables while lifecycle-managed generations or persisted workflows depend on them.

If rollout must be reversed, restore the complete set of predeployment database backups and the matching predeployment application version together. Restore the key material that encrypts those databases as part of the same recovery. The restore returns the system to the backup point: connections, logical bindings, or token generations created afterward are lost and may require users to reconnect, rebind workflows, or reauthorize with the provider. Do not combine predeployment databases with a postdeployment key or vice versa. If the required backups or matching key are unavailable, stop and escalate; do not run migration `Down` or delete managed secret rows to force an older binary to start.
