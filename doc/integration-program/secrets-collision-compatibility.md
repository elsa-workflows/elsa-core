...
The resolver design explains the observation. [`HostTenantResolver`](../../src/modules/Elsa.Tenants.AspNetCore/Resolvers/HostTenantResolver.cs) can map `Request.Host` to a tenant's `Http:Host` configuration, but [`MultitenantHttpRoutingFeature`](../../src/modules/Elsa.Tenants.AspNetCore/Features/MultitenantHttpRoutingFeature.cs) only registers it; the configured [`DefaultTenantResolverPipelineInvoker`](../../src/modules/Elsa.Tenants/Services/DefaultTenantResolverPipelineInvoker.cs) uses the first resolver that returns a known tenant and does not compare host and principal. The reference [`Elsa.Server.Web`](../../src/apps/Elsa.Server.Web/Program.cs) selects `CurrentUserTenantResolver` after authentication; that resolver obtains the tenant from the authenticated stored user. In this configuration a host alias is not an authorization or connection-environment boundary. A deployment that requires one tenant per host must add an explicit host/principal consistency policy and verify mismatch rejection; enabling a global rejection implicitly would change shared-host and custom resolver behavior. Logical connection environment binding is enforced separately by the Connections lifecycle contract. This is a source-pinned policy boundary, not a claim that every Elsa host chooses the same resolver pipeline.
