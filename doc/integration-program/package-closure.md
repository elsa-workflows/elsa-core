...
Preflight also rejects any inventory project marked packable (or with unknown packability) whose package ID is missing. The current inventory records `test/TlsSmoke/TlsSmoke.csproj` as MSBuild-evaluated packable but lacks its ID, so this earlier gate now stops source execution before graph evaluation. It is not silently omitted from the source-owned package set. The final consolidated inventory must record evaluated package identities before this proof can pass. The earlier recorded clean-CI restore failures and transitive-edge diagnosis remain valid historical evidence, not a successful current preflight.
