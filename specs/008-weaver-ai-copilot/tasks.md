...
- [ ] T001 [P] Create AI abstractions project in `src/modules/Elsa.AI.Abstractions/Elsa.AI.Abstractions.csproj`.
- [ ] T002 [P] Create AI host project in `src/modules/Elsa.AI.Host/Elsa.AI.Host.csproj`.
- [ ] T003 [P] Create Copilot adapter project in `src/modules/Elsa.AI.Copilot/Elsa.AI.Copilot.csproj`.
- [ ] T004 [P] Create EF Core persistence project in `src/modules/Elsa.AI.Persistence.EFCore/Elsa.AI.Persistence.EFCore.csproj`.
- [ ] T005 [P] Create abstractions unit test project in `test/unit/Elsa.AI.Abstractions.UnitTests/Elsa.AI.Abstractions.UnitTests.csproj`.
- [ ] T006 [P] Create host unit test project in `test/unit/Elsa.AI.Host.UnitTests/Elsa.AI.Host.UnitTests.csproj`.
- [ ] T007 [P] Create Copilot unit test project in `test/unit/Elsa.AI.Copilot.UnitTests/Elsa.AI.Copilot.UnitTests.csproj`.
- [ ] T008 [P] Create EF Core persistence unit test project in `test/unit/Elsa.AI.Persistence.EFCore.UnitTests/Elsa.AI.Persistence.EFCore.UnitTests.csproj`.
- [ ] T009 [P] Create AI integration test project in `test/integration/Elsa.AI.IntegrationTests/Elsa.AI.IntegrationTests.csproj`.
- [ ] T010 Add AI projects and tests to `Elsa.sln`.
- [ ] T011 [P] Add abstractions global usings in `src/modules/Elsa.AI.Abstractions/Usings.cs`.
- [ ] T012 [P] Add host global usings in `src/modules/Elsa.AI.Host/Usings.cs`.
- [ ] T013 [P] Add Copilot global usings in `src/modules/Elsa.AI.Copilot/Usings.cs`.
- [ ] T014 [P] Add EF Core persistence global usings in `src/modules/Elsa.AI.Persistence.EFCore/Usings.cs`.
- [ ] T015 [P] Add unit test global usings in `test/unit/Elsa.AI.Host.UnitTests/Usings.cs`.
- [ ] T016 [P] Add integration test global usings in `test/integration/Elsa.AI.IntegrationTests/Usings.cs`.
...
- [ ] T017 [P] Add conversation, session, message, and stream event models in `src/modules/Elsa.AI.Abstractions/Models/AiConversation.cs`.
- [ ] T018 [P] Add context attachment and resolved context models in `src/modules/Elsa.AI.Abstractions/Models/AiContextAttachment.cs`.
- [ ] T019 [P] Add tool definition, invocation, result, and enablement models in `src/modules/Elsa.AI.Abstractions/Models/AiToolDefinition.cs`.
- [ ] T020 [P] Add proposal models with durable lifecycle fields in `src/modules/Elsa.AI.Abstractions/Models/AiProposal.cs`.
- [ ] T021 [P] Add durable audit event models in `src/modules/Elsa.AI.Abstractions/Models/AiAuditEvent.cs`.
- [ ] T022 [P] Add provider and orchestrator contracts in `src/modules/Elsa.AI.Abstractions/Contracts/IAiProvider.cs`.
- [ ] T023 [P] Add tool registry and tool contracts in `src/modules/Elsa.AI.Abstractions/Contracts/IAiTool.cs`.
- [ ] T024 [P] Add context provider contract in `src/modules/Elsa.AI.Abstractions/Contracts/IAiContextProvider.cs`.
- [ ] T025 [P] Add conversation store, proposal store, and audit sink contracts in `src/modules/Elsa.AI.Abstractions/Contracts/IAiProposalStore.cs`.
- [ ] T026 [P] Add AI host and provider options for retention, reconnect grace, result limits, and BYOK provider references in `src/modules/Elsa.AI.Host/Options/AiHostOptions.cs`.
- [ ] T027 [P] Add AI permissions in `src/modules/Elsa.AI.Host/Permissions/AiPermissions.cs`.
- [ ] T028 Add service registration extension in `src/modules/Elsa.AI.Host/Extensions/ServiceCollectionExtensions.cs`.
- [ ] T029 Add module feature registration in `src/modules/Elsa.AI.Host/Features/AiFeature.cs`.
- [ ] T030 Add shell feature registration in `src/modules/Elsa.AI.Host/ShellFeatures/AiFeature.cs`.
- [ ] T031 [P] Implement configurable in-memory conversation store in `src/modules/Elsa.AI.Host/Services/InMemoryAiConversationStore.cs`.
- [ ] T032 [P] Implement tool enablement service in `src/modules/Elsa.AI.Host/Services/AiToolEnablementService.cs`.
- [ ] T033 [P] Implement audit sink fan-out in `src/modules/Elsa.AI.Host/Services/AiAuditSink.cs`.
- [ ] T034 [P] Add EF Core AI DbContext in `src/modules/Elsa.AI.Persistence.EFCore/DbContext.cs`.
- [ ] T035 [P] Add EF Core proposal entity mapping in `src/modules/Elsa.AI.Persistence.EFCore/Entities/AiProposalRecord.cs`.
- [ ] T036 [P] Add EF Core audit entity mapping in `src/modules/Elsa.AI.Persistence.EFCore/Entities/AiAuditRecord.cs`.
- [ ] T037 Implement durable EF Core proposal store in `src/modules/Elsa.AI.Persistence.EFCore/Stores/EFCoreAiProposalStore.cs`.
- [ ] T038 Implement durable EF Core audit sink in `src/modules/Elsa.AI.Persistence.EFCore/Stores/EFCoreAiAuditSink.cs`.
- [ ] T039 Add EF Core persistence feature and shell feature registration in `src/modules/Elsa.AI.Persistence.EFCore/ShellFeatures/AiPersistenceFeature.cs`.
- [ ] T040 [P] Add registration and naming tests in `test/unit/Elsa.AI.Host.UnitTests/AiRegistrationTests.cs`.
- [ ] T041 [P] Add tool metadata and enablement validation tests in `test/unit/Elsa.AI.Abstractions.UnitTests/AiToolDefinitionTests.cs`.
- [ ] T042 [P] Add durable proposal persistence tests in `test/unit/Elsa.AI.Persistence.EFCore.UnitTests/EFCoreAiProposalStoreTests.cs`.
- [ ] T043 [P] Add durable audit persistence tests in `test/unit/Elsa.AI.Persistence.EFCore.UnitTests/EFCoreAiAuditSinkTests.cs`.
- [ ] T044 [P] Add provider isolation boundary tests in `test/unit/Elsa.AI.Copilot.UnitTests/CopilotBoundaryTests.cs`.
...
- [ ] T045 [P] [US1] Add context authorization tests in `test/unit/Elsa.AI.Host.UnitTests/Context/AiContextResolverTests.cs`.
- [ ] T046 [P] [US1] Add stream event mapping tests in `test/unit/Elsa.AI.Host.UnitTests/Streaming/AiStreamEventMapperTests.cs`.
- [ ] T047 [P] [US1] Add reconnect grace tests in `test/unit/Elsa.AI.Host.UnitTests/Streaming/AiReconnectGraceTests.cs`.
- [ ] T048 [P] [US1] Add chat endpoint streaming integration tests in `test/integration/Elsa.AI.IntegrationTests/AiChatEndpointTests.cs`.
- [ ] T049 [P] [US1] Add capabilities endpoint integration tests in `test/integration/Elsa.AI.IntegrationTests/AiCapabilitiesEndpointTests.cs`.
- [ ] T050 [P] [US1] Add tools endpoint integration tests in `test/integration/Elsa.AI.IntegrationTests/AiToolsEndpointTests.cs`.
- [ ] T051 [P] [US1] Add disconnected chat recovery integration tests in `test/integration/Elsa.AI.IntegrationTests/AiChatReconnectTests.cs`.

### Implementation for User Story 1

- [ ] T052 [P] [US1] Implement workflow definition context provider in `src/modules/Elsa.AI.Host/Context/WorkflowDefinitionContextProvider.cs`.
- [ ] T053 [P] [US1] Implement workflow instance context provider in `src/modules/Elsa.AI.Host/Context/WorkflowInstanceContextProvider.cs`.
- [ ] T054 [US1] Implement context resolver with authorization and redaction in `src/modules/Elsa.AI.Host/Context/AiContextResolver.cs`.
- [ ] T055 [US1] Implement AI tool registry with enablement filtering in `src/modules/Elsa.AI.Host/Services/AiToolRegistry.cs`.
- [ ] T056 [US1] Implement orchestrator turn flow in `src/modules/Elsa.AI.Host/Services/AiOrchestrator.cs`.
- [ ] T057 [US1] Implement reconnect grace tracking in `src/modules/Elsa.AI.Host/Streaming/AiStreamSessionManager.cs`.
- [ ] T058 [US1] Implement stream event mapper in `src/modules/Elsa.AI.Host/Streaming/AiStreamEventMapper.cs`.
- [ ] T059 [US1] Implement chat endpoint in `src/modules/Elsa.AI.Host/Endpoints/Ai/Chat/Endpoint.cs`.
- [ ] T060 [US1] Implement tools endpoint in `src/modules/Elsa.AI.Host/Endpoints/Ai/Tools/Endpoint.cs`.
- [ ] T061 [US1] Implement capabilities endpoint in `src/modules/Elsa.AI.Host/Endpoints/Ai/Capabilities/Endpoint.cs`.
- [ ] T062 [US1] Implement Copilot provider event adapter in `src/modules/Elsa.AI.Copilot/Adapters/CopilotProvider.cs`.
- [ ] T063 [US1] Implement Copilot feature and shell feature registration in `src/modules/Elsa.AI.Copilot/ShellFeatures/CopilotAiFeature.cs`.
- [ ] T064 [US1] Draft paired Studio Razor chat panel implementation, after reviewing the `elsa-extensions` `origin/feat/ai` UI prototype at commit `93f0e09d71e57f5daff1e2d593f0a51faaa80417`, in `../elsa-studio/src/modules/Elsa.Studio.AI/UI/Components/WeaverChatPanel.razor`.
- [ ] T065 [US1] Run chat MVP tests in `test/integration/Elsa.AI.IntegrationTests/Elsa.AI.IntegrationTests.csproj`.
