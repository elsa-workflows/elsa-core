...
**Independent Test**: Start a long-running workflow whose burst takes several seconds to complete. Issue a stop signal to the host. Verify that the host does not exit until the burst has completed and the instance is persisted at its natural suspension point, and that no new bursts were dispatched after the stop signal.

**Acceptance Scenarios**:

1. **Given** a workflow instance is executing a burst of activities, **When** the host receives a stop signal, **Then** the burst runs to completion and the instance reaches a persisted suspension, completion, or fault state before the process exits.
2. **Given** external event ingress is actively delivering messages into the runtime, **When** the host receives a stop signal, **Then** ingress sources pause before the drain waits for active bursts, and no new bursts start after the pause point.
3. **Given** an active burst has not completed by the drain deadline, **When** the deadline elapses, **Then** the affected instances are cancelled, marked with a distinguishable interruption state, and forensic details are recorded before the process exits.
...
**Independent Test**: Invoke the pause API against a running runtime. Verify that ingress sources transition to `Paused`, that workflow instances resulting from already-in-flight bursts continue to completion, that no new bursts begin, and that invoking the resume API restores normal operation including drainage of any stimuli buffered during the pause.

**Acceptance Scenarios**:

1. **Given** the runtime is in normal operation, **When** an authorised administrator invokes the pause action, **Then** every ingress source transitions to `Paused` (or `PauseFailed` with a visible error), already-running bursts continue to completion, and no new bursts begin.
...
4. **Given** the runtime is paused, **When** the status action is invoked, **Then** the response identifies the overall quiescence state, the reason (administrative pause), per-ingress-source state, and the count of active bursts.
...
When a drain exceeds its deadline or an operator issues a force-stop, workflows that were mid-burst at that moment are force-cancelled. On the next host start (or next shell activation), these instances should recover automatically, without operator intervention, and should be distinguishable in dashboards and metrics from workflows that died in an ungraceful crash.
...
**Independent Test**: Force a drain deadline breach on an active burst. Verify that the affected instance is marked with the interrupted sub-status, that a forensic entry is written to the per-instance execution log, and that on next shell activation the instance is requeued and resumed without waiting for the existing timeout-based recovery cadence.

**Acceptance Scenarios**:

1. **Given** a burst is force-cancelled during drain, **When** persistence completes before process exit, **Then** the instance carries the interrupted sub-status and an execution log entry describing the interruption.
...
- A burst schedules additional activities in a tight loop, never naturally reaching suspension. Drain deadline bounds the wait; force-cancel applies.
- An ingress source acknowledges its pause but continues to deliver messages due to a buggy implementation. The runtime detects this via per-burst ingress attribution and flips the source's recorded state to `PauseFailed`.
...
- The persistence layer is unavailable at the moment of drain. Bursts that complete and cannot be persisted transition into the interrupted sub-status by policy, and the failure is surfaced in the drain outcome.
...
**Burst-of-execution lifecycle**

- **FR-014**: The runtime MUST maintain a registry of active bursts of execution, including which ingress source initiated each burst where that attribution is available.
- **FR-015**: During drain, active bursts MUST be permitted to run to their next natural persistence boundary (suspension, completion, or fault) within the drain deadline.
- **FR-016**: The drain process MUST wait for the active-burst count to reach zero before allowing the host to exit, bounded by the drain deadline.
- **FR-017**: When the drain deadline elapses or an operator-initiated force is issued, the runtime MUST cancel outstanding bursts, persist the affected instances in the interrupted sub-status, record forensic detail, and proceed to termination.
- **FR-018**: A burst started by an ingress source that claims to be in the `Paused` state MUST cause the runtime to flip that source's recorded state to `PauseFailed` and surface the inconsistency in the status output.

**Interruption and recovery**

- **FR-019**: The workflow sub-status model MUST include an interrupted value distinct from suspended, cancelled, and faulted. It MUST represent "last burst was force-cancelled by the runtime; the instance remains resumable."
- **FR-020**: Every force-cancelled burst MUST record a forensic entry in the existing per-instance execution log, using a stable event name and a typed payload that includes interruption time, reason, shell generation identifier, and the identifier of the last active activity.
...
- **FR-032**: The runtime MUST expose an authenticated and authorised action to query status, returning the composite quiescence state and reason(s), per-ingress-source state and last error, and the active-burst count.
...
- **FR-034**: Administrative pause and resume events MUST be audited.
...
- **Burst Handle**: A handle representing one in-flight burst of execution. Carries the initiating workflow instance identifier and, where available, the name of the ingress source that caused the burst to start.
- **Interrupted Sub-Status**: A new value in the existing workflow sub-status enumeration, denoting an instance whose last burst was force-cancelled by the runtime and is awaiting recovery.
...
- **Drain Outcome**: The structured result of a completed drain, including overall status, elapsed time for each phase, per-source final state with any captured error, and the count of bursts force-cancelled at the deadline.
...
- Deployments that run a distributed cluster tolerate individual nodes refusing new work while draining; cross-node handoff of in-memory burst state is explicitly out of scope.
...
- **SC-001**: In validation scenarios, 100% of active bursts that can complete within the configured drain deadline do so before the host exits, and the corresponding workflow instances reach a persisted suspension, completion, or fault state.
- **SC-002**: In validation scenarios, 100% of bursts that cannot complete within the drain deadline result in instances marked with the interrupted sub-status and a corresponding forensic entry in the per-instance execution log.
...
- **SC-005**: Administrative pause stops all registered ingress sources from initiating new bursts within their configured pause timeouts, and administrative resume restores normal operation with accumulated stimuli dispatched in order.
- **SC-006**: Pausing one shell in a multi-shell deployment does not affect the active-burst throughput of any other shell on the same host.
...
- **SC-009**: A buggy ingress source that acknowledges pause but continues to deliver messages is detected within the first subsequent burst it initiates, and its recorded state transitions to `PauseFailed`.
