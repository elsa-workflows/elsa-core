...
Owns the drain protocol end-to-end: parallel pause of every `IIngressSource`, wait for `BurstRegistry.ActiveCount` to reach zero within the drain deadline, force-cancel + mark-`Interrupted` on breach, emit the forensic `WorkflowInterrupted` event per affected instance, and return a `DrainOutcome`.
...
3. **Wait for bursts** — poll `IBurstRegistry.ActiveCount` on a short interval (10 ms) until it reaches zero OR the drain deadline elapses.
4. **On deadline breach** — iterate live `BurstHandle`s; for each:
   - Cancel `BurstCts`.
   ...
   - If persistence itself fails, still mark the burst as interrupted in memory and record `Reason = "PersistenceFailure"` in the payload (the instance will likely be recovered later by the timeout-based `RestartInterruptedWorkflowsTask`).
...
3. **No new bursts after step 1** — after `BeginDrainAsync` returns, any attempt to start a burst via the dispatcher is rejected. Internal workers skip their next dequeue; external ingress sources observe their own `PauseAsync` signal. This is the property that makes step 3's "wait for bursts to reach zero" terminate.
...
- `BurstRegistry` is the new accounting layer. Every call site that currently dispatches work — `IStimulusDispatcher`, `IWorkflowRunner.RunAsync`, `IWorkflowRestarter` — registers a `BurstHandle` on entry and disposes it on exit. These are the three choke points; no other types create bursts.
